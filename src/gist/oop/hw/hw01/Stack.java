...
class Stack {
	...
	private int toploc ; // 삽입과 삭제가 발생하는 스택 top을 가르킴. 즉, 저장된 문자
						 // 데이터의 개 수 정보를 갖는 변수
	...
	char pop() {
		if (toploc == 0) { //스택이 비어 있는지check
			System.out.println("\n- 스택이 비어 있음 -");
			return (char)0;
		}

		return s[--toploc] ; // 스택 top에서 문자를 return시키고 top을 감소시킴 }
	}
